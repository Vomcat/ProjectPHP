...
<div class="divTableCellHead">Panel</div>
</div>
<?php
while($row = $result->fetch_assoc())
{
	echo '<div class="divTableRow"><div class="divTableCell">' . $row["nazwa"] . '</div><div class="divTableCell">' . $row["data_rozpoczecia"] . '</div><div class="divTableCell">' . $row["powierzchnia"] . '</div><div class="divTableCell">' . $row["podpowierzchnia"] . '</div><div class="divTableCell">' . $row["rozmiar"] . ' m<sib>2</sib></div><div class="divTableCell">' . $row["roslina"] . '</div><div class="divTableCell">' . $row["nawoz"] . '</div><div class="divTableCell"><form action="z_doswiadczenie.php" method="POST"><input type="text" name="z_doswiadczenie" value="' . $row["idd"] . '" hidden><button type="submit">Zakończ doświadczenie</button></form></div></div>';
}
?>
